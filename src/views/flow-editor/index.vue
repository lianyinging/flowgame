...
import { executeFlowGameStreamApi } from '@/api/flow-game'
import type { FlowStreamEventName } from '@/api/flow-game'
import { buildKbBaseSelectOptions } from '@/api/flow-game/kb-collection'
import { listKbBasesCached } from '@/api/flow-game/qdrant'
import { buildFlowRedisKey, flowRedisKeysForLoad, parseFlowNameFromRedisKey } from '@/api/flow-game/constants'
import { getRedisApi, saveFlowWorkflowApi } from '@/api/flow-game/redis'
import useLoading from '@/hooks/useLoading.ts'
import { flowGameCustomNodes } from './custom-nodes'
import { initialData } from './initial-data.ts'
import { syncMethodKeyInWorkflow } from './sync-method-key.ts'
import {
  hasStartApiNode,
...
} from './workflow-start-api-rules.ts'
import { normalizeKnowledgeNodeParams } from './normalize-knowledge-node-params.ts'
import { collapseAllNodePanels, ensureNodeExpandDefault } from './normalize-node-expand.ts'
import { normalizeLlmApiNodeParams } from './normalize-llmapi-node-params.ts'
import { patchCanvasControlsPosition } from './patch-canvas-controls-position.ts'
import { patchCanvasMinimapStyle, setCanvasMinimapVisible } from './patch-canvas-minimap-style.ts'
import { patchCanvasNodePopover } from './patch-canvas-node-popover.ts'
import { patchCanvasWatermark } from './patch-canvas-watermark.ts'
import {
  cleanupNodeInspectorTrigger,
  FLOWGAME_OPEN_NODE_INSPECTOR_EVENT,
  patchNodeInspectorTrigger
} from './patch-node-inspector-trigger.ts'
import {
  FLOWGAME_OPEN_FLOW_KNOWLEDGE_EVENT,
  FLOWGAME_OPEN_FLOW_LIST_EVENT,
  patchFlowToolbarVariables
} from './patch-flow-toolbar-variables.ts'
import FlowListPanelModal from './FlowListPanelModal.vue'
import FlowKnowledgePanelModal from './FlowKnowledgePanelModal.vue'
import type { FlowListIndexItem } from '@/api/flow-game/redis'
...
} from './tinyflow-flow-api.ts'
import VariableTreeContent from './VariableTreeContent.vue'
import NodeInspectorPanel from './NodeInspectorPanel.vue'
import CanvasFloatingToolbar from './CanvasFloatingToolbar.vue'
import FlowRunProgressModal from './FlowRunProgressModal.vue'
import { buildWorkflowRunPlan } from './build-workflow-run-plan.ts'
import {
  isFlowRunFailed,
  parseFlowRunSummary,
  parseNodeExecutions,
...
} from './parse-flow-execute-result.ts'
import type { FlowNodeExecution, FlowRunViewState } from './flow-run-types.ts'
import type { FlowParameter, InspectorFlowNode } from './node-inspector-config.ts'
import './index.scss'

const route = useRoute()
const isViewMode = computed(() => route.query.mode === 'view')
const pageTitle = computed(() => {
  const redisKey = typeof route.query.redisKey === 'string' ? route.query.redisKey.trim() : ''
  const name = redisKey
...
async function openFlowFromListPanel(payload: { mode: IFormType, record?: FlowListIndexItem }) {
  if (payload.mode === 'add') {
    saveForm.flowName = ''
...
async function loadWorkflowData() {
  const redisKeyFromQuery = typeof route.query.redisKey === 'string' ? route.query.redisKey.trim() : ''
  const nameFromQuery = typeof route.query.name === 'string' ? route.query.name.trim() : ''
  const redisKey = redisKeyFromQuery || (nameFromQuery ? buildFlowRedisKey(nameFromQuery) : '')

  if (!redisKey)
    return initialData

  if (nameFromQuery)
    saveForm.flowName = nameFromQuery
  else if (redisKeyFromQuery)
    saveForm.flowName = parseFlowNameFromRedisKey(redisKeyFromQuery)
