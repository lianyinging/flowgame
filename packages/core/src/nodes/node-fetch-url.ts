...
/**
 * URL 批量抓取：对齐 demo FetcherAgent —— 优先 Jina Reader，失败再 HTML strip。
 * 入参 urls（数组/单字符串/文档列表）；输出 documents + 首条便捷字段。
 */
export const nodeFetchUrl: CustomNode = {
  title: '网页抓取',
  description:
    '批量抓取 URL：优先 Jina 抽取可读正文，失败则降级 HTML 纯文本；输出 documents 列表',
  sortNo: 420,
  group: 'base',
  icon: FETCH_URL_ICON,
  ...
  forms: [
    nodeFormHeading('抓取设置', 'fetch-url-settings'),
    {
      ...
      description: '每条 URL 正文超过部分截断，默认 6000'
    }
  ]
}
