...
import {
  mountLlmApiProviderModelPicker,
  updateLlmApiProviderModelPicker
} from './llmapi-provider-model-picker'
...
/**
 * 模型调用：按预置厂家 + 模型名 + API Key 调用（OpenAI 兼容）。
 * 不允许自定义接口地址。
 */
export const nodeLlmApi: CustomNode = {
  title: '模型调用',
  description: '选择模型厂家与模型，填写 API Key 即可调用（OpenAI 兼容）',
  sortNo: 350,
  group: 'base',
  icon: LLM_ICON,
  ...
  forms: [
    // 画布：heading + render 自定义联动下拉；侧栏：LlmApiProviderModelBlock
    // 厂家 / 模型 / apiKey 均可在「输入参数」改为引用；表单变更会同步到 fixed 入参
    nodeFormHeading('模型厂家与名称', 'llmapi-provider-model'),
    {
      type: 'input',
      name: 'apiKey',
      label: 'API Key',
      placeholder: 'sk-... 或 {{apiKey}}',
      defaultValue: '',
      description: '可填明文或 {{apiKey}}；执行时用同名入参替换。密钥会写入流程 JSON，请注意权限'
    },
    {
      type: 'input',
  ...
  render: (parent, node, flow) => {
    void mountLlmApiProviderModelPicker(parent, node, flow)
  },
  onUpdate: (parent, node) => {
    updateLlmApiProviderModelPicker(parent, node)
  }
}
