...
/**
 * 网页搜索：对齐 demo_ai_news Scout —— Google News/RSS + DuckDuckGo（可选 Wikipedia）。
 * 均为免费、无需 API Key；engines 存于 node.data.engines（string[]）。
 */
export const nodeWebSearch: CustomNode = {
  title: '网页搜索',
  description: 'Google News/RSS + DuckDuckGo 免费检索（无需 API Key），输出 documents / title / content / url',
  sortNo: 410,
  group: 'base',
